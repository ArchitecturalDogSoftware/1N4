...
impl Parse for DefaultEqExpr {
    fn parse(input: ParseStream) -> Result<Self> {
        // Not actually an identifier, but it's close enough.
        let default: Ident = input.parse()?;
        if default != "default" {
            return Err(Error::new(default.span(), "expected `default`"));
        }
...
impl FieldsWithDefaults {
    ...
    ///   [`Self::ident`], which wraps each field it in [`Some`] (or calls that type's implementation of the method).
...
/// Represents the two ways a value can have its default value generated.
